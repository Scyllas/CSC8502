...
Mesh * Mesh::GenerateQuad(float width, float height) {
	...
	m->vertices[0] = Vector3(-width / 2, -height / 2, 0.0f);
	m->vertices[1] = Vector3(-width / 2, height / 2, 0.0f);
	m->vertices[2] = Vector3(width / 2, -height / 2, 0.0f);
	m->vertices[3] = Vector3(width / 2, height / 2, 0.0f);

	float texsize = 250;
	float h = height / texsize;
	float w = width / texsize;

	m->textureCoords[0] = Vector2(0.0f,h);
	m->textureCoords[1] = Vector2(0.0f, 0.0f);
	m->textureCoords[2] = Vector2(w, h);
	m->textureCoords[3] = Vector2(w, 0.0f);
